...
function handle_filesystem_api_toggle() {
    // Verify nonce
...
function handle_openkbs_callback(WP_REST_Request $request) {
...
function handle_openkbs_polling() {
...
function create_encrypted_payload($content, $chatTitle, $app) {   
    return array(
        'kbId' => $app['kbId'],
        'apiKey' => $app['apiKey'],
        'chatTitle' => $chatTitle,
        'encrypted' => true,
        'message' => encrypt_kb_item(json_encode(array(
            'type' => 'API_REQUEST',
            'data' => $content,
...
function openkbs_publish($data, $chatTitle) {
    ...
    foreach ($apps as $app) {
        ...
        if (isset($app['wp_actions'][$event])) {
            ...
            $payload = create_encrypted_payload($data, $chatTitle, $app);
